...
const MoltenMetal = ({
  ...
  mouseStrength = 0.3,
  opacity = 1.0,
  // --- new perf-oriented props ---
  dpr = 1.5,          // was capped at 2; lower = big fragment-shader win
  targetFPS = 0,       // 0 = uncapped, or e.g. 30 to throttle on low-end devices
  className = ''
}) => {
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      antialias: false,
      depth: false,      // no depth buffer needed for a full-screen quad
    ...
    const doResize = () => {
      ...
      if (w === lastW && h === lastH) return; // skip redundant work
      lastW = w;
      lastH = h;
    ...
    const setSize = () => {
      // batch bursts of ResizeObserver callbacks into one rAF
    ...
    doResize(); // initial size, synchronous so first frame isn't blank
    ...
    const loop = t => {
      ...
      // optional FPS throttle for low-power / battery-saving mode
    ...
    tryStart();
    renderer.render({ scene: mesh }); // paint first frame immediately
